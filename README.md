...
- 仿真通过，但是上板测试没有通过
  ...
  - ### 问题的核心：异步读 vs 同步读 (RAM)

    1. **`Ram.v` (仿真模型):**
       - 你的写入逻辑是**同步**的 (`always @(posedge CLK ...)`） 。这很好。
       - 你的读取逻辑是**异步**的 (`always @(*)`） 。
    2. **FPGA上的BRAM (硬件实现):**
       - 当你综合代码时，`reg [7:0] RAM[0:1023]`  会被工具（如Vivado或Quartus）推断为片上块存储器（**BRAM**）。
       - FPGA上的BRAM（或任何同步RAM）的读取操作是**同步**的。它需要一个时钟周期才能完成。

    ### 仿真与硬件的行为差异

    #### 1. 仿真（为什么能成功）

    当你的CPU执行 `lbu a0, 0(s0)`  指令时：

    1. **执行周期 (第 N 拍):**
       - ALU计算出地址 (例如 `0x10000004`)。
       - `Bus.v` 将地址和 `cs=1`, `wmem=0` 信号发送给 `Ram.v`。
       - `Ram.v` 的 `always @(*)` 块**立即（组合逻辑）** 响应，从 `RAM[0x10000004]` 中读出数据 `1`，并将其放到 `Do_Ram` 输出上 。
       - `Do_Ram` 的值流经总线和多路选择器，在时钟上升沿**之前**到达 `RegisterFile` 的 `Data` 输入端。
    2. **时钟上升沿 (第 N 拍结束):**
       - `RegisterFile` 将 `Data`（即 `1`）写入 `a0` 寄存器。
       - **结果：** 仿真成功，`a0` 得到了 `1`。

    #### 2. 硬件（为什么会失败）

    当你的CPU在FPGA上执行 `lbu a0, 0(s0)`  指令时：

    1. **执行周期 (第 N 拍):**
       - ALU计算出地址 (`0x10000004`)。
       - `Bus.v` 将地址和 `cs=1`, `wmem=0` 信号发送给 **BRAM**。
    2. **时钟上升沿 (第 N 拍结束):**
       - **BRAM（硬件）** 看到地址和控制信号，**锁存这个地址**。
       - **CPU**（按照 `ControlUnit` 的设计）期望数据**此时**已经准备好，`RegisterFile` 尝试锁存 `Do_Ram` 上的值。
       - 但此时 `Do_Ram` 上的值是**上一个周期**的（或者是 `0`），因为BRAM还没有输出新的数据。
       - **结果：** `a0` 寄存器被写入了 `0`。
    3. **执行周期 (第 N+1 拍):**
       - **BRAM** 现在才将 `0x10000004` 地址处的数据 `1` 输出到 `Do_Ram` 上。
       - 但为时已晚，`ControlUnit` 和 `RegisterFile` 已经在处理下一条指令了。

    **总结：你的CPU流水线设计（单周期加载）与FPGA硬件（同步RAM）的物理特性不匹配。** 仿真模型（异步RAM）掩盖了这个问题。
